...
namespace SHRJ
{
    ...
    //    {
    //        int designId;
    //        string type = context.Request.QueryString["type"] ?? "base";

    //        if (!int.TryParse(context.Request.QueryString["id"], out designId))
    //        {
    //            RedirectNoImage(context);
    //            return;
    //        }

    //        string dbPath = GetImagePath(designId, type);

    //        // fallback hover → base
    //        if (string.IsNullOrEmpty(dbPath) && type == "hover")
    //            dbPath = GetImagePath(designId, "base");
    ...
    //        context.Response.RedirectPermanent(imageUrl, false);
    //    }

    //    private void RedirectNoImage(HttpContext context)
    //    {
    //        context.Response.Redirect("http://skote.runasp.net/Images/no-image.jpg", false);
    //    }

    //    private string GetImagePath(int designId, string type)
    //    {
    //        string cacheKey = $"DESIGN_IMG_{designId}_{type}";
    ...
    //              AND (
    //                    (@Type = 'base' AND IsBase = 1)
    //                 OR (@Type = 'hover' AND IsMBHover = 1)
    //              )";
    ...
    //        if (!string.IsNullOrEmpty(path))
    //            HttpRuntime.Cache.Insert(cacheKey, path, null,
